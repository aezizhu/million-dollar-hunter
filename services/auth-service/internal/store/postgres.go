...
func (s *PGStore) Create(ctx context.Context, email, passwordHash string) (User, error) {
	...
	var u User
	err := s.Conn.QueryRow(ctx, q, email, passwordHash).Scan(&u.ID, &u.Email, &u.PasswordHash, &u.CreatedAt, &u.UpdatedAt)
	return u, err
}
...
func (s *PGStore) GetByEmail(ctx context.Context, email string) (User, error) {
	...
	var u User
	err := s.Conn.QueryRow(ctx, q, email).Scan(&u.ID, &u.Email, &u.PasswordHash, &u.CreatedAt, &u.UpdatedAt)
	if err != nil {
		return User{}, err
	}
	return u, nil
